            ...
            <tbody id="cart-tbody">
              <?php foreach ($cart as $id => $qty):
                ...
                <tr data-id="<?= htmlspecialchars($id) ?>">
                      ...
                      <div class="item-img">
                        <?php if (!empty($p['image'])): ?>
                          <img src="<?= htmlspecialchars($p['image']) ?>" alt="<?= htmlspecialchars($p['name']) ?>"
                            loading="lazy" class="cart-img">
                          <div class="item-img-fb item-img-fb--fallback" style="display:none">
                            <?= htmlspecialchars(mb_substr($p['name'], 0, 1)) ?>
                          </div>
                        <?php else: ?>
                  ...
                  <td>
                    <div class="qty-wrap">
                      <button class="qty-btn" id="dec-<?= htmlspecialchars($id) ?>"
                        onclick="updateQty('<?= htmlspecialchars($id) ?>', -1)" aria-label="Decrease quantity" <?= $qty <= 1 ? 'disabled' : '' ?>>−</button>
                      <input type="number" class="qty-num" id="qty-<?= htmlspecialchars($id) ?>" value="<?= $qty ?>" min="1"
                        max="99" aria-label="Quantity" onchange="setQty('<?= htmlspecialchars($id) ?>', this.value)">
                      <button class="qty-btn" id="inc-<?= htmlspecialchars($id) ?>"
                        onclick="updateQty('<?= htmlspecialchars($id) ?>', 1)" aria-label="Increase quantity" <?= $qty >= 99 ? 'disabled' : '' ?>>+</button>
                    </div>
                  </td>
        ...
        <div>
          <div class="summary-box">
            ...
            <div class="summary-body">
              ...
              <div class="summary-line"><span>VAT (20%)</span><span id="sum-tax">$<?= number_format($tax, 2) ?></span>
              </div>

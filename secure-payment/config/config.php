...
define('APP_ENV', 'development'); // switch to 'production' on live server
...
spl_autoload_register(function ($class) {
    // Only handle classes in the Vault namespace
    if (strpos($class, 'Vault\\') === 0) {
        $name = str_replace('Vault\\', '', $class);
        $file = __DIR__ . '/../src/' . str_replace('\\', '/', $name) . '.php';
        // Use include_once for the autoloader to avoid fatal errors if file is missing
        if (file_exists($file)) {
            include_once $file;
        }
    }
